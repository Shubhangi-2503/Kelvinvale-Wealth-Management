...
| **Customer** | View owned accounts/holdings, place instructions (Subscription, Withdrawal, Switch), update profile[cite: 1]. | Reaches **only self-owned records**[cite: 1]. Cannot provision products or access another customer's money[cite: 1]. |
...
* **SIPP Restrictions:** Validated via the `IProductOpeningRule` strategy (minimum opening age: 18)[cite: 1]. Withdrawals prior to the statutory minimum pension age (55) are rejected upfront with `400 Bad Request`[cite: 1].
...
* **Stored Procedures & Audit Trail:** Critical read models and high-throughput transactional flows utilize compiled SQL Stored Procedures (`sp_GetCustomerPortfolioOverview`, `sp_DeactivateCustomerCascade`, `sp_AuditLogLedger`). All mutations stamp `CreatedById`, `CreatedOn`, `ModifiedById`, and `ModifiedOn`.
